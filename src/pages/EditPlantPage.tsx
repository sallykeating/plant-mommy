...
export default function EditPlantPage() {
  ...
  async function handleSpeciesSelect(item: PerenualSpeciesListItem) {
    ...
    if (item.default_image?.regular_url && !pendingFile && !plant?.photo_url) {
    ...
    if (mapped.length) {
      if (!sunlightPreference) setSunlightPreference(mapped[0]);
      if (!currentLight) setCurrentLight(mapped[0]);
    }
